...
const API_KEY = process.env.MusicAidAPI002

export async function POST(request: NextRequest) {
  try {
    console.log(
      "[v0] Available env vars:",
      Object.keys(process.env).filter((key) => key.includes("Music")),
    )
    console.log("[v0] MusicAidAPI002 value:", process.env.MusicAidAPI002 ? "SET" : "NOT SET")
    console.log(
      "[v0] All env keys containing 'API':",
      Object.keys(process.env).filter((key) => key.includes("API")),
    )

    if (!API_KEY) {
      console.error("MusicAidAPI002 environment variable is not set")
