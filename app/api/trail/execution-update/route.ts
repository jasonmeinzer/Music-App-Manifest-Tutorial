...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!response.ok) {
      ...
      console.error("Execution update API request failed:", response.status, errorText)
      return NextResponse.json({ error: `API request failed: ${response.status}` }, { status: response.status })
    }

    const data = await response.json()
    console.log("[v0] Server-side execution update response:", data)

    return NextResponse.json(data)
  } catch (error) {
    console.error("Server-side execution update error:", error)
